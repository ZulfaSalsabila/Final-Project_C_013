...
import webservice_c.uas_c.exceptions.NonexistentEntityException;
import webservice_c.uas_c.exceptions.PreexistingEntityException;
...
public class MahasiswaJpaController implements Serializable {
    ...
    private EntityManagerFactory emf = Persistence.createEntityManagerFactory("webservice_c_uas_c_jar_0.0.1-SNAPSHOTPU");
    ...
    public void create(Mahasiswa mahasiswa) throws PreexistingEntityException, Exception {
        if (mahasiswa.getMahasiswaPK() == null) {
            mahasiswa.setMahasiswaPK(new MahasiswaPK());
        }
        ...
        try {
            ...
            if (findMahasiswa(mahasiswa.getMahasiswaPK()) != null) {
    ...
    public void edit(Mahasiswa mahasiswa) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                MahasiswaPK id = mahasiswa.getMahasiswaPK();
    ...
    public void destroy(MahasiswaPK id) throws NonexistentEntityException {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
    ...
    public Mahasiswa findMahasiswa(MahasiswaPK id) {
        EntityManager em = getEntityManager();
        try {
    ...
    Mahasiswa findMahasiswa(SingularAttribute<MahasiswaPK, Integer> nik) {
